...
已确认维持**应用身份**（`tenant_access_token`）写入，取舍如下：

- 所有成员提交的修改在表格版本历史、单元格编辑记录中统一显示为应用名称；分发的 `app_id`/`app_secret` 全员相同，服务端视角为同一操作者，无法区分具体成员；
- 将来如需按成员溯源，可升级为 user_access_token（OAuth 授权码流程：浏览器授权回调换 token，约 2 小时过期，refresh_token 续期），本期不实现。
...
- **表格列结构与提交格式**：线上表格除 A~E 外还有 F 列（COUNTIF 公式）与日期列格式约束，提交时需一并处理，详见「表格列结构与写入格式」。

## 表格列结构与写入格式

实测线上表格与早期规划存在三处差异，已据此实现：

- **F 列 COUNTIF 公式**：表格 F 列为 `=COUNTIF(C$2:C{n},C{n})`，统计该条目所属制作组织的第几部作品。提交时在 A~E 之外追加写入 F 列公式；行号按「数据行数 + 2 + i」推算（数据从 A2 起、表头占第 1 行），依赖表格连续无空洞，Rust 侧追加 range 扩展为 `A1:F{n}`；
- **日期列格式**：D、E 列为日期类型，原表显示形如 `2010/4/28`。直接写入 `2013-11-29` 文本会显示异常，故提交前将日期转为 Excel 序列号（`toExcelSerial`，与读取端 `excelDateToString` 互逆）以匹配列格式；
- **竖向边框**：A~F 列间有竖向实线边框，append 接口不自动继承单元格样式，短期内未实现单元格边框写入（见可选增强）。
...
- 创建时间精确到时分秒（当前仅日期粒度）；
- 追加行后通过单元格样式接口补齐竖向边框，或在飞书侧将列边框设为整列/数据区域样式（使新行自动带边框）。
