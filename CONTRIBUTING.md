...
应用界面支持简体中文与繁体中文（臺灣），基于 i18next 实现。用户在设置页的「界面语言」切换，选择持久化在 `settings.json` 的 `uiLanguage` 字段，启动时由 `initI18n()` 初始化。
...
2. 需要动态内容的，用 `{{}}` 插值，不要拼接字符串：
   ...
   ```
3. 在 [`src/i18n/zh-TW.ts`](/src/i18n/zh-TW.ts) 中补充对应的繁体映射，key 必须与简体原文完全一致；未收录的 key 会自动回退显示简体，不会白屏

注意事项：

- 已禁用 `keySeparator` 和 `nsSeparator`，避免原文中的冒号、点号被误解析为分隔符
- 不要在业务代码里自行做简繁转换或手写语言映射表
- 语言选项名称（如「简体中文」「繁體中文（臺灣）」）按各自语言展示，不参与转换
...
- 考虑到语言普及程度~~以及我的技术水平~~，后端Rust主要处理一些网络请求、底层逻辑等，在数据组织等场景如果没有性能压力都应放在前端TypeScript。
- 使用AI辅助编程时注意读取[AGENTS.md](AGENTS.md)，并且原则上AI生成的每一行代码都要人工审查。
